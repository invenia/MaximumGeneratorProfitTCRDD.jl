...
Detailed description: The algorithm is composed by two parts, (1) the screening loop and (2)
the bisection loop. The screening loop uses the initial bid0 and creates an aproximation of 
the profit function. Using this aproximation, it identifies the range [bid lower (bid_lo) 
and bid higher (bid_hi)] in which the bid that maximises the generator profit could be.
Once the lower and upper bid range are identified, the bisection loop will find try to find
if the approximations of the lower and upper bid intersect at some point. if they do, the
intersection bid is used to find a closer upper and lower range, or even if the optimum has
been found. If they dont intersect then, traditional bisection is done until the bid that 
maximises the generators profit is found [1].
...
"""
function MaxGenProfit_tcrdd(sys::System, bid0::Float64; dual_lines_tol::Float64 = 1e-1, 
    dual_gen_tol::Float64 = 1e-1, segm_bid_argmax_profit::Int64 = 50000, 
    maxit_scr::Int64 = 5, maxit_bi::Int64 = 30, epsilon::Float64 = 0.01,
    print_results::Bool =true, print_progress::Bool=true, print_plots::Bool = true, 
    ...
    if bid0 < Pmin_orig || bid0 > Pmax_orig
        error("Initial Bid (bid0) must be within Pmin: ",Pmin_orig,
    ...
    # ----------Evaluate Profit----------
    (profit_argmax0, bid_argmax0) = bid_argmax_profit(sys, BaseMVA, lmp0, 
        tcrdd_slack0, bid0, Pmin_orig, Pmax_orig; segm_bid_argmax_profit, print_plots)
    ...
    elseif bid0 == Pmin_orig && bid_argmax0 < Pmin_orig
        ...
        while (iter_scr <= maxit_scr && stop == false)
            ...
            if iter_scr > 1 #only if we need to recalculate because we are still searching
                ...
                set_active_power_limits!(gen_thermal_slack,(min = Pmin_orig, max = bid0))
                # Solve PTDF OPF
                (lmp0, res0) = opf_PTDF(sys)
                # Calculate TCRDD
                tcrdd_slack0 = f_TCRDD(sys, res0)
                # Evaluate Profit
                (profit_argmax0, bid_argmax0) = bid_argmax_profit(sys, BaseMVA, lmp0, 
                    tcrdd_slack0, bid0, Pmin_orig, Pmax_orig; segm_bid_argmax_profit, 
                    print_plots)
            end
            if bid_argmax0 > bid0
            ...
            # Evaluate Profit
            (profit_argmax1, bid_argmax1) = bid_argmax_profit(sys, BaseMVA, lmp1, 
                tcrdd_slack1, bid1, Pmin_orig, Pmax_orig; segm_bid_argmax_profit, 
                print_plots)
            # step 5 LSL
            if bid_argmax1 == bid1
                bid_opt_found = true
            ...
            elseif ((bid_argmax1-bid1)*(bid_argmax0-bid0)) < 0
                ...
                # Go To !bisection_loop
                (stop, iter_bi, bid_opt_found, bid_opt, bid_mid) = bisection_loop(sys, BaseMVA, bid_lo, bid_hi,
                Pmin_orig, Pmax_orig, bid_opt_found, stop; maxit_bi, network, solver, print_plots,
                segm_bid_argmax_profit, epsilon, print_progress)
                break
            else

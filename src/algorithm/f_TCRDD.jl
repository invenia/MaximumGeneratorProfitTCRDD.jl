"""
    function f_TCRDD(args...; kwargs...)

Calculates the Transmission Constrained Residual Demand Derivative (TCRDD) of the slack
bus of an optimised power system (optimised with OPF minimising total generation cost).

Detailed description: The TCRDD is calculated using the PTDF matrix of the system. This
matrix is divided in accordance with the generator segments (specified below) and load
buses as well as the binding lines of the solved system [1]. To determine if a generator
or a branch are binding, the dual variables from the OPF are compared against a tolerance.
The tolerance for binding lines and generators can be specified individually.

Generator Segments:
    1) Slack Generator
    2) Generators with binding active power
    3) Generators with non zero slopes in their cost function
    4) Generators with only constant price in their cost function
...
function f_TCRDD(
    ...
    nonzeroslope_nb_PTDF = get_PTDF_thermal_nonzeroslope(
        sys,
        res,
        PTDF_binding_lines;
        dual_gen_tol
    )
    gens_thermal_nonzeroslope_busnumber, PTDFb_nonzeroslope = nonzeroslope_nb_PTDF
    ...
    zeros_nonzeroslope_constprice = zeros(
        ngens_thermal_nonzeroslope,
        ngens_thermal_constprice
    )
    ...
    if nbind_lines ≠ 0  && ngens_thermal_constprice ≠ 0
        ...
        @constraint(model_a,const_a,
            A * [dPgnonzeroslope_dlam_a; dPgconstprice_dlam_a; dmubind_dlam_a] .== b)
        ...
        #Calculate the TCRDD
        tcrdd_slack = -((ones_nonzeroslope') * dPgnonzeroslope_dlam_a_value) -
            ((ones_constprice') * dPgconstprice_dlam_a_value)

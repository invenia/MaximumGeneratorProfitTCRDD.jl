...
function disp_plots(
    ...
    ns = length(gen_virt_names)
    gen_profit_both = Array{Float64}(undef, (div + 1, ns + 1));
    gen_profit_both[:, [1:1:ns;]] = virt_profit_s;
    gen_profit_both[:, ns+1] = virt_profit_tot;
    plot(bids, gen_profit_both , title = "Virt Generators Profits",
        label = [gen_virt_names; "total"], legend = :topleft);
    xlabel!("Bid q(pu)");
    a = ylabel!("profit(\$/hr)")
    display(a)

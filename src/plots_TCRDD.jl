...
Transmission Constrained Residual Demand Derivative (TCRDD) approach to maximise 
the slack generator profit vs the actual behaviour of the system under different
ammounts of bids.

Detailed description: The function runs a series of Optimal Power Flows (OPFs)
for a series of bids which range from the minimum (Pmin) power of the slack 
generator to the maximum power of the slack generator (Pmax) for a set number 
of samples. Then for each sample the values of the Profit, Cost, LMP, TCRDD,
Residual demand and Binding Lines are calculated and plotted.
The TCRDD algorithm is taken from [1]
...
"""
function plots_TCRDD(sys::System; div::Int64 = 1000, network::DataType = StandardPTDFModel,
    solver = optimizer_with_attributes(Ipopt.Optimizer), tcrdd_tol::Float64 = 1e-3,
    gap::Int64 = 50, dual_lines_tol::Float64 = 1e-1, dual_gen_tol::Float64 = 1e-1)
    ...
    # ----------Only for Project Plots----------
    branch1_3= Array{Float64}(undef,div+1) # <-DELETE ME
    ...
    for bid in Bidmin:step:Bidmax #minimum bid to maximum bid
        ...
        gen_cost_slack[iter_opf] = α*(Pg_slack[iter_opf]*BaseMVA)^2 + β*(Pg_slack[iter_opf]*BaseMVA) + γ
        ...
        branch1_3[iter_opf] = res.variable_values[:Fp__Line][1,"Line1"]# <-DELETE ME
        ...
        if iter_opf == 1
            ...
            #----
            lmp_slack_apx_tcrdd[iter_opf] = ( (1/tcrdd0)*( Pg_slack[iter_opf] - Pg_slack0 ) ) + lmp_slack0;
            clearprice_apx_tcrdd = lmp_slack_apx_tcrdd[iter_opf]*Pg_slack[iter_opf]*BaseMVA
    ...
    #----------Plots----------
    disp_plots(bids, gen_profit, gen_profit_tcrdd, gen_profit_noCost,
        gen_profit_tcrdd_noCost, gen_cost_slack, Pg_slack, lmp_slack, 
        lmp_slack_apx_tcrdd, residualD_slack_Pg, tcrdd, nbind_lines, branch1_3)
    return(nothing)
end
